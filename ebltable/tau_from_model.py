...
class OptDepth(object):
    ...
    def __init__(self, z, EGeV, tau, kx=2, ky=2):
        ...
        self.__tauSpline = RBSpline(
            self._logEGeV, self._z, self._tau, kx=kx, ky=ky)
        return
        ...
        if model == 'kneiske' or model.find('dominguez') >= 0 or model == 'finke':
            if model == 'kneiske':
                file_name = os.path.join(
                    ebl_file_path, 'tau_ebl_cmb_kneiske.dat')
            elif model == 'dominguez':
                file_name = os.path.join(
                    ebl_file_path, 'tau_dominguez11_cta.out')
            elif model == 'dominguez-upper':
                file_name = os.path.join(
                    ebl_file_path, 'tau_upper_dominguez11_cta.out')
            elif model == 'dominguez-lower':
                file_name = os.path.join(
                    ebl_file_path, 'tau_lower_dominguez11_cta.out')
            elif model == 'finke':
    ...
    def writefits(self, filename, z, ETeV):
        ...
        hdulist = fits.HDUList(
            [fits.PrimaryHDU(), fits.table_to_hdu(t), fits.table_to_hdu(t2)])
    ...
    def opt_depth(self, z, ETeV):
        ...
        if np.any(z < self._z[0]):
            warnings.warn(
                "Warning: a z value is below interpolation range, zmin = {0:.2f}".format(
                    self._z[0]),
                RuntimeWarning)
        ...
        tt[args_z, :] = self.__tauSpline(
            np.log10(np.sort(ETeV)*1e3), np.sort(z)).transpose()
        result[:, args_E] = tt
    ...
    def opt_depth_Ebin(self, z, Ebin, func, params, Esteps=50):
        ...
        for i, E in enumerate(Ebin):
            if i == len(Ebin) - 1:
                break
            logE_array.append(np.linspace(
                np.log(E), np.log(Ebin[i+1]), Esteps))
        ...
        return simps(func(np.exp(logE_array), **params) * t_array * np.exp(logE_array), logE_array, axis=1) / \
            simps(func(np.exp(logE_array), **params) *
                  np.exp(logE_array), logE_array, axis=1)
